...
async def get_scorecard(scorecard_id: int) -> Dict:
    ...
    scorecard_id = scorecard[0]
    title = scorecard[1]
    criteria = json.loads(scorecard[2])
    status = scorecard[3]
    
    # Check if any criteria need updating (Relevance pass score and min_score range)
    needs_update = False
    updated_criteria = []
    ...
    for criterion in criteria:
        ...
        # Check if this is a Relevance criterion with old pass score (3)
        if (criterion.get("name", "").lower() == "relevance" and 
            criterion.get("pass_score") == 3):
            updated_criterion["pass_score"] = 4
            needs_update = True
        
        # Update min_score from 1 to 0 for stricter scoring
        if criterion.get("min_score") == 1:
            updated_criterion["min_score"] = 0
            needs_update = True
        
        updated_criteria.append(updated_criterion)
    
    # If criteria were updated, save the changes to the database
    if needs_update:
        await execute_db_operation(
            f"UPDATE {scorecards_table_name} SET criteria = ? WHERE id = ?",
            (json.dumps(updated_criteria), scorecard_id),
        )
