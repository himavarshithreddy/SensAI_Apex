...
from api.config import openai_plan_to_model_name, USE_GOOGLE_SERVICES
...
def get_user_audio_message_for_chat_history(uuid: str) -> List[Dict]:
    if USE_GOOGLE_SERVICES:
        return get_user_audio_message_for_chat_history_google(uuid)
    ...
    try:
        ...
        if isinstance(transcription_result, dict):
            ...
            return [
                {
                    "type": "text",
                    "text": f"Student's Response: {transcription_result}",
                }
            ]
        ...
        return [
            ...
            {
                "type": "text", 
                "text": "Student's Response: [Audio transcription failed]",
            }
        ]
...
async def ai_response_for_question(request: AIChatRequest):
    ...
    # Handle user message - audio returns multiple messages
    if request.response_type == ChatResponseType.AUDIO:
        audio_messages = get_user_audio_message_for_chat_history(request.user_response)
        # For now, use the AI analysis message (second message) for the chat history
        # The first message (transcription) will be handled separately in streaming
        if len(audio_messages) >= 2:
            user_message = {"role": "user", "content": audio_messages[1]["text"]}
        else:
            user_message = {"role": "user", "content": audio_messages[0]["text"] if audio_messages else "Audio processing failed"}
    else:
        user_message = {"role": "user", "content": get_user_message_for_chat_history(request.user_response)}
    ...
    async def stream_response() -> AsyncGenerator[str, None]:
        logger.info("[stream_response] Entered stream_response generator")
        with tracer.start_as_current_span(
            ...
            if request.task_type == TaskType.LEARNING_MATERIAL:
                with using_attributes(
                    ...
                    logger.info("[stream_response] Preparing system prompt for LEARNING_MATERIAL")
                    ...
                    logger.info("[stream_response] Calling run_llm_with_instructor for query_rewrite")
                    ...
                    logger.info("[stream_response] Received rewritten query from LLM")
            ...
            try:
                logger.info(f"[stream_response] response_type: {request.response_type}")
                format_instructions = ""  # Always define before use
                
                # For audio responses, yield the transcription message first
                if request.response_type == ChatResponseType.AUDIO:
                    audio_messages = get_user_audio_message_for_chat_history(request.user_response)
                    if len(audio_messages) >= 1:
                        transcription_message = audio_messages[0]["text"]
                        yield json.dumps({"feedback": transcription_message}) + "\n"
                        logger.info("[stream_response] Yielded transcription message for audio response")
                ...
                if request.response_type == ChatResponseType.AUDIO:
                    ...
                    with using_attributes(
                        ...
                        logger.info("[stream_response] Calling run_llm_with_instructor for router decision")
                        ...
                        logger.info(f"[stream_response] Router decision: use_reasoning_model={router_output.use_reasoning_model}")
                    if router_output.use_reasoning_model:
                        model = openai_plan_to_model_name["reasoning"]
                    else:
                        model = openai_plan_to_model_name["text"]

                logger.info(f"[stream_response] Using model: {model}")
                ...
                logger.info("[stream_response] Preparing system prompt and messages for feedback stage")
                if request.task_type == TaskType.QUIZ:
                    knowledge_base = None
                    if question["context"]:
                ...
                with using_attributes(
                    ...
                    logger.info("[stream_response] Streaming response from LLM...")
                    # Process the async generator
                    async for chunk in stream:
                        content = json.dumps(chunk.model_dump()) + "\n"
                        output_buffer.append(content)
                        logger.info("[stream_response] Yielding chunk to client")
                        yield content
                logger.info("[stream_response] Finished streaming all chunks")
            except Exception as error:
                logger.error(f"[stream_response] Exception occurred: {error}")
                span.record_exception(error)
                span.set_status(Status(StatusCode.ERROR))
                # Optionally yield an error message to the client for debugging
                yield json.dumps({"error": str(error)}) + "\n"
                raise
            else:
                logger.info("[stream_response] Streaming completed successfully")
