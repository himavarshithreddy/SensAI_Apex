            ...
            if (responseType === 'audio' && audioData) {
                ...
                try {
                    ...
                    if (!presignedUrlResponse.ok) {
                        // If S3 is not configured (503) or any other error, fall back to local upload
                        if (presignedUrlResponse.status === 503) {
                    ...
                    // Continue to local upload fallback
                }
                ...
                .then(response => {
                    ...
                    const processStream = async () => {
                        try {
                            ...
                            // Track if we've shown the transcription to user
                            let hasShownTranscription = false;
                            // Track if we're processing audio response
                            const isAudioResponse = responseType === 'audio';
                            ...
                            while (true) {
                                ...
                                for (const line of jsonLines) {
                                    try {
                                        ...
                                        // For audio responses, handle transcription display first
                                        if (isAudioResponse && !hasShownTranscription && data.feedback && 
                                            data.feedback.includes('🎤 Your Transcription:')) {
                                            hasShownTranscription = true;
                                            
                                            // Stop the AI responding animation immediately
                                            ...
                                            setChatHistories(prev => ({
                                                ...prev,
                                                [currentQuestionId]: [...(prev[currentQuestionId] || []), transcriptionMessage]
                                            }));

                                            // Start AI responding animation again for the analysis
                                            setIsAiResponding(true);
